...
struct LocationResult {
    ...
    int32_t sourceType;
    int32_t errorCode;  // 0 表示成功
    LocationCallbackCtx* ctx;
};

// ---- 在 JS 线程上执行 resolve/reject ----
...
static void OnLocationUpdate(Location_Info* location, void* userData)
{
    ...
    // 构建结果，通过 threadsafe function 传到 JS 线程
...
static napi_value NativeGetCurrentLocation(napi_env env, napi_callback_info info)
{
    ...
    // 创建 threadsafe function 用于跨线程回调到 JS 线程
    napi_value workName;
    napi_create_string_utf8(env, "NativeLocationCallback", NAPI_AUTO_LENGTH, &workName);
